...
vector<string> username(MAXUSR, "(no name)");
vector<string> ip_address(MAXUSR, "");
vector<int> port(MAXUSR, 0);
vector<int> client_fd(MAXUSR, -1);

typedef struct user_pipe_pair{
    int user_pipe[2];
}user_pipe;

vector<map<string, string>> user_env(MAXUSR);
map<string, user_pipe> current_user_pipe;
fd_set all_fd, selected_fd;
int dev_null_input = open("/dev/null", O_RDONLY);
int dev_null_output = open("/dev/null", O_WRONLY);

// npshell variables
vector<int> line_cnt(MAXUSR, 0);
int numbered_pipe[MAXUSR][2*MAXPIPE];
vector<vector<vector<pid_t>>> pid_wait(MAXUSR, vector<vector<pid_t>>(MAXPIPE));
vector<vector<bool>> numpiped_to(MAXUSR, vector<bool>(MAXPIPE, false));   // store if the current numbered pipe is piped by some command or not (default set to false)
